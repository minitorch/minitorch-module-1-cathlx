...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert sigmoid(a - 1) < sigmoid(a + 1) if - \
        6 <= a <= 6 else sigmoid(a - 1) <= sigmoid(a + 1)
...
def test_distribute(x: float, y: float, z: float) -> None:
    ...
    """
    assert_close(z * add(x, y), add(z * x, z * y))
...
def test_associativity(x: float, y: float, z: float) -> None:
    ...
    Test that operators are associative (i. e. (a + b) + c = a + (b + c))."""
